            ...
            if (Auth::user()->role_id == 3) {
                ...
                $nearByBarbers = User::where(['users.is_available' => 1, 'users.is_active' => 1, 'users.is_service_added' => 1, 'users.role_id' => 2])->select(
                    ...
                    DB::raw("3959 * acos(cos(radians(" . $latitude . "))
                                 * cos(radians(users.latest_latitude)) * cos(radians(users.latest_longitude) - radians(" . $longitude . "))
                                 + sin(radians(" . $latitude . ")) * sin(radians(users.latest_latitude))) AS distance")
                );
